...
import Popper, { commonPopperModifiers } from "src/styles/styled-mui/StyledPopper";
...
const ProgressPopper: React.FC<IProgressPopper> = ({
  ...
  const handleDownloadClick = (file: IDownloadableFile) => {
    ...
    const filteredFiles = receivedFiles!.filter((prevFile) => prevFile.fileBlobUrl !== file.fileBlobUrl);
  ...
  return (
    ...
    >
      <div className="MuiPopper-arrow" ref={setArrowRef} />
      <PopperContentWrapper>
        {progressType === "receive" ? (
          <>
            <ContentTitle>
